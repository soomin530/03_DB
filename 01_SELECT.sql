...
SELECT '같음'
FROM DUAL
WHERE 1 = '1'; -- WHERE(조건절), "=" : 같다 --> 문자열 '1'을 숫자 1로 변환

-- 문자열 타입이어도 저장된 값이 숫자면 자동으로 형변환하여 연산 가능
SELECT EMP_ID + 10 FROM EMPLOYEE;
-- '200'+10 = NUMBER타입으로 변환되어 210으로 연산됨.
...
FROM DUAL;
-- 날짜에 +/- 연산 시 일 단위로 계산이 진행됨(전날, 다음날) 

--------------------------------------------------------------------

-- <컬럼 별칭 지정>

/* 컬럼명 AS 별칭 : 별칭 띄어쓰기 있으면 X, 특수문자도 X, 문자만 O
 * 
 * 컬렴명 AS "별칭" : 별칭 내 띄어쓰기 O, 특수문자도 X, 문자만 O
 * 
 * AS는 생략 가능
 * */
...
-- 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC(기본) | DESC] [NULLS FIRST | LAST]
                                  -- (오름차순)(내림차순)

-- EMPLOYEE 테이블에서 (FROM 절)
-- 급여가 3백만 원 초과인 사원의 (WHERE 절)
...
-- EMPLOYEE 테이블에서(FROM절) 부서코드가 'D9'인 사원의(WHERE절)
-- 사번, 이름, 부서코드(DEPT_CODE), 직급코드(JOB_CODE)를 조회(SELECT절)
...
-- EMPLOYEE 테이블에서
-- 급여가 300만 원 미만 또는 500만 원 이상인 사원의 
-- 사번, 이름, 급여, 전화번호 조회
...
-- EMPLOYEE 테이블에서 입사일이 1990-01-02 ~ 1999-12-31 사이인 직원의
-- 이름, 입사일 조회
...
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%'; -- % : '전'으로 시작하는 사람의 이름

-- EMPLOYEE 테이블에서 전화번호가 010으로 시작하지 않는 사원
-- 사번, 이름, 전화번호 조회
SELECT EMP_ID, EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%'; -- 전화번호가 010으로 시작하지 않는

-- EMPLOYEE 테이블에서 EMAIL_ 앞에 글자가 세 글자인 사원만
-- 이름, 이메일 조회
-- SELECT * FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서
-- 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6'이고 --> AND 가 OR보다 우선순위가 높음! () 사용 가능
...
OR DEPT_CODE IS NULL; -- 14명 (NULL 포함)
...
FROM EMPLOYEE
ORDER BY SALARY ASC; -- ASC(오름차순)가 기본 값이기 때문에 생략 가능 [ASC | DESC]
...
ORDER BY 3 DESC; -- 컬럼명 대신 컬럼 순서도 사용 가능!

-- 입사일 순서대로 이름, 입사일 조회(별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일
FROM EMPLOYEE
ORDER BY 입사일; -- 컬럼명 대신 별칭도 사용 가능!
-- ORDER BY 해석순서보다 SELECT 해석순서가 빠르기 때문에
-- ORDER BY에 별칭 사용 가능!
