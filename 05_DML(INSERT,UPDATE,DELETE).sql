...
-- 주의 : 혼자서 COMMIT, ROLLBACK을 하지 말 것 !
...
VALUES('900','김민수', '981012-1234567', 'msm_1012@or.kr', '01012345678', 'D1', 
		'J7', 'S3', 4300000, 0.2, 200, SYSDATE, NULL, 'N'/* = DEFAULT*/);
...
-- 	  선택이 안 된 컬럼 값은 NULL이 들어감 (DEFAULT 존재 시 DEFAULT 설정한 값으로 삽입됨) 

INSERT INTO EMPLOYEE2 (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, 
						DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
			VALUES('900', '김민수', '981012-1234567', 'msm_1012@or.kr', 
					'01012345678', 'D1', 'J7', 'S3', 4300000);
SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900';

COMMIT; -- 김민수 데이터 영구 저장
ROLLBACK; -- ROLLBACK 수행함

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900'; -- 조회 다시 해봄(영구저장 되었기 때문에 롤백 해도 되돌리기 안 됨)
...
SELECT * FROM DEPARTMENT2; -- 전부 기술연구팀으로 업데이트됨
...
DEPT_ID = 'D0',
DEPT_TITLE = '전략기획팀' -- WHERE절 직전 코드 작성 시 콤마(,) 작성 X
WHERE DEPT_ID = 'D9'
AND DEPT_TITLE = '총무부';
...
SELECT * FROM EMP_M01; -- M02에 있던 곽두원 M01로 INSERT
...
SELECT * FROM EMPLOYEE2
WHERE EMP_NAME = '김민수'; --> 조회 결과 있음

-- EMPLOYEE2 테이블 전체 삭제하기
DELETE FROM EMPLOYEE2;
--> 24행 삭제
