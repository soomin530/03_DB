...
WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE
				   WHERE EMP_NAME = '노옹철'); -- 수행하고 싶은 부분 드래그 후 alt + x = 그 부분만 수행!
...
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE -- SELECT 절에서 DEPT_TITLE 을 조회하려고 하기 때문에
 ...
 *
 * - > ALL, < ALL : 여러 개의 결과값의 모든 값 보다 큰 / 작은 경우
...
FROM EMPLOYEE 
WHERE SALARY IN (SELECT MAX(SALARY) -- 다중행이기 때문에 IN 사용 (비교연산자 X)
			  	 FROM EMPLOYEE
			   	 GROUP BY DEPT_CODE)
ORDER BY DEPT_CODE;
...
-- 3) 위의 두 쿼리를 합쳐 사수에 해당하는 직원에 대한
-- 	  사번, 이름, 부서명, 직급명, 구분('사수'로) 조회
...
SELECT EMP_NAME, DEPT_CODE FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID FROM DEPARTMENT -- 다중행이라 IN 사용
WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION WHERE NATIONAL_CODE = 'KO')); -- 단일행이라 비교연산자 사용
				

--------------------------------------------------------------------------------------

-- 3. (단일행) 다중열 서브쿼리
-- 서브쿼리 SELECT 절에 나열된 컬럼 수가 여러 개일 때
...
FROM EMPLOYEE WHERE DEPT_CODE = (SELECT DEPT_CODE
								FROM EMPLOYEE WHERE ENT_YN = 'Y'
								AND SUBSTR(EMP_NO, 8, 1) = '2')	
...
-- 사번, 이름, 직급코드, 급여 조회
-- 단, 급여와 급여 평균은 만 원 단위로 계산 = TRUNC(컬럼명, -4)							
...
-- 상관쿼리
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, MANAGER_ID
FROM EMPLOYEE MAIN LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-- 퇴사자인 이태림을 제외한 상태로 D8부서의 가장 빠른 입사자도 포함
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음') JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여와의(급여 차)차를 조회
...
-- 단, 관리자가 없는 경우 '없음'으로 표시 (관리자는 매니저)
-- 스칼라 + 상관쿼리 이용
...
WHERE EMP_ID = MANAGER_ID; -- MANAGER_ID랑 EMP_ID가 같음
...
-->   FROM 절에 작성되기 때문에 해석 순위 1순위
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;

-- 2) 메인쿼리 조회 시 ROWNUM 5 이하까지만 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM(SELECT EMP_NAME, SALARY
	 FROM EMPLOYEE
	 ORDER BY SALARY DESC) -- 해석순위 1순위 -> FROM절에서 전체 직원의 SALARY 내림차순 정렬
WHERE ROWNUM <= 5; -- 해석순위 2순위 -> WHERE절에서 가상컬럼의 1~5행까지만 조회
--> 전체 직원에서 급여 TOP5 구함!
...
--               ex) 공동 1위가 2명이면 다음 순위 2위가 아니라 3위가 조회
...
-- 2. 고용일이 2000년도 이후인 사원들 중 급여가 가장 높은 사원의 (서브)
--    사번, 사원명, 전화번호, 급여, 직급명을 조회하시오. (메인)
...
WHERE (DEPT_CODE, MANAGER_ID) = (SELECT DEPT_CODE, MANAGER_ID
								FROM EMPLOYEE
								WHERE EMP_NO LIKE '77%' -- 다른 방법 : WHERE SUBSTR(EMP_NO, 1, 2) = '77' 
								AND SUBSTR(EMP_NO, 8, 1) = '2');
...
SELECT MIN(HIRE_DATE)
FROM EMPLOYEE 
WHERE ENT_YN = 'N' -- 또는 AND ENT_YN != 'Y'
GROUP BY DEPT_CODE;
...
FLOOR(MONTHS_BETWEEN(SYSDATE , TO_DATE(SUBSTR(EMP_NO,1 ,6), 'RRMMDD')) / 12) "나이",
TO_CHAR(SALARY * (1 + NVL(BONUS, 0)) * 12, 'L999,999,999') "보너스 포함 연봉"
-- FROM EMPLOYEE MAIN JOIN JOB USING(JOB_CODE)                 -> USING 못 쓰기 때문에
FROM EMPLOYEE MAIN JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE) --> 이렇게 변경!
...
-- SQL 문법 규칙 : USING절은 JOIN을 할 때 특정 컬럼을 명시할 때만 사용됨
-- 				   중요한 규칙은 USING절에 명시된 컬럼은 중복되지 않으며, 별칭 없이 바로 사용해야함. 


-- 다른 방법 (위 방법보다 좀 더 간결함)
SELECT EMP_ID, EMP_NAME, JOB_NAME,
FLOOR(MONTHS_BETWEEN(SYSDATE , TO_DATE(SUBSTR(EMP_NO,1 ,6), 'RRMMDD')) / 12) "나이",
TO_CHAR(SALARY * (1 + NVL(BONUS, 0)) * 12, 'L999,999,999') "보너스 포함 연봉"
FROM EMPLOYEE MAIN JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
...
FROM EMPLOYEE
WHERE SALARY >= 3000000 AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6')
