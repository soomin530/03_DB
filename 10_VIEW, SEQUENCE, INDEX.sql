...
-- V_DCOPY2 라는 VIEW를 이용해서 INSERT 수행
INSERT INTO V_DCOPY2
VALUES('D0', 'L2');
--> 가상 테이블 VIEW 인데 INSERT 성공
...
--  VIEW를 생성할 떄 사용된 컬럼에만 데이터가 삽입되었고
 ...
 *             단, 시퀀스가 생성 되자마자 호출할 경우 오류 발생.   -- NEXTVAL 사용 후 CURRVAL 사용해야 함
...
-- SQL Error [904] [42000]: ORA-00904: "SEQ_TEST_NO"."CURRVAL": 부적합한 식별자
...
-- TB_IDX_TEST 테이블에
-- 샘플 데이터 100만 개 삽입 (PL/SQL 사용) 
-- 조건문(IF/CASE), 반복문(LOOP, WHILE), 변수 선언
BEGIN
	FOR I IN 1..1000000 -- I는 1부터 100만 까지 반복
	LOOP 
...
SELECT COUNT(*) FROM TB_IDX_TEST; -- 100만 개 행이 삽입됨을 확인
...
WHERE TEST_ID = 'TEST500000'; -- 0.021s
