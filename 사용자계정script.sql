...
CREATE USER kh_wsm IDENTIFIED BY kh1234;
-- 계정 생성 구문 (ID : kh_wsm / PW : kh1234) 

GRANT RESOURCE, CONNECT TO kh_wsm;
-- 사용자 계정 권한 부여 설정
-- RESOURCE : 테이블이나 인덱스 같은 DB 객체를 생성할 권한
-- CONNECT : DB에 연결하고 로그인할 수 있는 권한

ALTER USER kh_wsm DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
