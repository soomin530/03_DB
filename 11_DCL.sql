...
CREATE USER wsm_sample IDENTIFIED BY 1234;


-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 WSM_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한........ TO 사용자명;
GRANT CREATE SESSION TO wsm_sample;
...
GRANT CREATE TABLE TO wsm_sample; -- 테이블 생성할 권한 줌
ALTER USER wsm_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
--> 해당 계정은 지정된 권한 이용해서 특정 역할을 갖게 된다

-- (sys) sample 계정에 CONNECT, RESOURCE ROLE 부여
GRANT CONNECT, RESOURCE TO wsm_sample;
...
-- 2. (kh) kh sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO wsm_sample;
...
REVOKE SELECT ON EMPLOYEE FROM wsm_sample;
